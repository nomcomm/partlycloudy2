...
def percentage_larger_elements(vector, number):
    count_larger = sum(1 for element in vector if element > number)
    total_elements = len(vector)
    
    if total_elements == 0:
        return 0  # Avoid division by zero
    
    percentage = (count_larger / total_elements)
    if percentage==0:
        percentage = 1/len(vector)
    return percentage
...
def partly_isc_matrix(data, pairwise=True, summary_statistic=None, verbose=True):
    """Intersubject correlation
    ----------
    data : list or ndarray
        fMRI data for which to compute ISC
...
def partly_permutation_isc(iscs, group_assignment=None, pairwise=False,  # noqa: C901
